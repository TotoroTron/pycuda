...
# ABSTRACT CLASS CUBLAS
class CuBlas(DotProduct):
        # https://github.com/numba/pyculib/blob/master/pyculib/tests/blas.py 
...
class CuBlasGemm(CuBlas):
    # https://pyculib.readthedocs.io/en/latest/cublas.html 
    def _dot(self, A, B, C):
        alpha = 1.0
        beta = 0.0
        m = A.shape[0]
        n = B.shape[1]
        k = A.shape[1]
        self._handle.gemm('N', 'N', m, n, k, A, B, alpha, beta, C) # 'N' means No transpose
